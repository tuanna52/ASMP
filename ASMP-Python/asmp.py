...
class Main_App(object):
    def __init__(self, master):
        ...
        #Header
        self.Header = Box (master, height = 90, width = "fill", border=None, align="top")
        self.Header.bg = "#242939"
        Text (self.Header, width=7, align="left")            #This is used to place elements properly
        Picture (self.Header, image = "mushroom.png", align = "left")
        Text (self.Header, text="Automation System for Mushroom Production", color="#5867dd", font="Arial Bold", size=25, height=90, align="left")

        #Footer
        self.Footer = Box (master, height=50, width="fill", border=None, align="bottom")
        ...
        Text (self.Side_Tab, height=3, align="top")           #This is used to place elements properly
        ...
        Text (self.Side_Tab, height=3, align="top")           #This is used to place elements properly
        self.Control_Button = PushButton (self.Side_Tab, text="Exit", pady=5, padx=17, align="top", command=self.exit_click)
        self.Control_Button.text_color = "#ffffff"
        self.Control_Button.text_size = 12
        self.Control_Button.font = "Arial Bold"
        self.Control_Button.bg = "#2d3244"
        ...
        Box (self.Dashboard, height=80, width=70, grid=[0, 0])    #This is used to place element properly

        self.Temp_Box = Box (self.Dashboard, height=230, width=190, grid=[1, 1])
        self.Temp_Box.bg = "#f9a2a2"
        Text (self.Temp_Box, text="Temperature (°C)", size=17, color="#303b6a", font="Arial", align="top").tk.config(pady=5)
        self.Temp_Value = Text (self.Temp_Box, height=2, text="0", size=45, color="#303b6a", font="Arial Bold", align="top")
        Picture (self.Temp_Box, image="temperature.png", align="top")

        Box (self.Dashboard, width=65, grid=[2, 0])              #This is used to place element properly

        self.Humid_Box = Box (self.Dashboard, height=230, width=190, grid=[3, 1])
        self.Humid_Box.bg = "#90e9eb"
        Text (self.Humid_Box, text="Humidity (%)", size=17, color="#303b6a", font="Arial", align="top").tk.config(pady=5)
        self.Humid_Value = Text (self.Humid_Box, height=2, text="0", size=45, color="#303b6a", font="Arial Bold", align="top")
        Picture (self.Humid_Box, image="humidity.png", align="top")

        Box (self.Dashboard, width=65, grid=[4, 0])              #This is used to place element properly

        self.CO2_Box = Box (self.Dashboard, height=230, width=190, grid=[5, 1])
        self.CO2_Box.bg = "#96f4b8"
        Text (self.CO2_Box, text="CO2 (ppm)", size=17, color="#303b6a", font="Arial", align="top").tk.config(pady=5)
        self.CO2_Value = Text (self.CO2_Box, height=2, text="0", size=45, color="#303b6a", font="Arial Bold", align="top")
        Picture (self.CO2_Box, image="co2.png", align="top")

        #Control page
        self.Control = Box (master, height="fill", width="fill", layout="grid", visible=False)
        Box (self.Control, height=80, width=70, grid=[0, 0])         #This is used to place element properly

        self.Temp_Ctr = Box(self.Control, height=230, width=190, grid=[1, 1])
        ...
        Box (self.Control, width=65, grid=[2, 0])                #This is used to place element properly

        self.Humid_Ctr = Box(self.Control, height=230, width=190, grid=[3, 1])
        ...
        Box (self.Control, width=65, grid=[4, 0])                #This is used to place element properly

        self.CO2_Ctr = Box(self.Control, height=230, width=190, grid=[5, 1])
...
# while 1:
#     try:
#         ser = serial.Serial(serial_port, 115200, timeout=0.1, writeTimeout=0.1)
#         break
#     except:
...
mainWindow = App (title = "Automation System for Mushroom Production", width=960, height=540, bg = "#ffffff")
